...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $sql = "SELECT * FROM versements WHERE id = :id";
    $stmt = $db->prepare($sql);
    $stmt->bindParam(':id', $id, PDO::PARAM_INT);
    $stmt->execute();
    $versements = $stmt->fetch(PDO::FETCH_ASSOC);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE versements SET montant = :montant WHERE id = :id";
    $stmt = $db->prepare($sql);
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active">versements / Modification</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-12">
              ...
              <div class="card">
                <div class="card-body">
                  <h5 class="card-title">Modifier un versements</h5>
              <form method="post" >
             
                <div class="row mb-3">
                  <label for="inputText" class="col-sm-2 col-form-label">montant:</label>
                  <div class="col-sm-10">
                    <input name="montant" type="text" class="form-control" value="<?= htmlspecialchars($versements['montant']) ?>" required>
                  </div>
                </div>
